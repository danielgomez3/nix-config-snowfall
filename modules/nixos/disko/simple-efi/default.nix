# modules/nixos/disko/simple-efi/disko-config.nix
{
  lib,
  pkgs,
  inputs,
  ...
  cfg = config.profiles.${namespace}.my.nixos.disko.simple-efi;

  inherit (lib) mkIf;
  inherit (lib.${namespace}) enabled mkBoolOpt mkOpt;
in {
  options.profiles.${namespace}.my.nixos.disko.simple-efi = {
    enable = mkBoolOpt false "Enable custom module for platform 'nixos', of category 'disko', of module 'simple-efi', for namespace '${namespace}'.";
